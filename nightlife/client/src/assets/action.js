...
const accountRequestFailure = ({ errorMsg, actionType }) => ({
...
export const submitAccountRequest = ({ username, password, actionType }) => {
	return dispatch => {
		...
		sendAccountRequest({ username, password, actionType })
			.then(accountResp => {
					if(accountResp.error){
						dispatch(accountRequestFailure({errorMsg: accountResp.error, actionType}));
					} else {
						let {userInfo, token} = accountResp;
						userInfo = userInfo || {};
						token = token || '';
